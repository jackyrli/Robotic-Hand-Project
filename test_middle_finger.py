...
#yellow before green, white before red
...
class finger:
    ...
    def control(self, timer):
        ...
        if self.encoder_count>= self.target[0] and self.encoder_count <= self.target[1]:
            ...
            ret = min(max(550, int(ret + 0.9 * self.error)), 660)
            #print('Bend PWM', ret)
            self.direction = 1
            self.bend(ret)
        elif self.encoder_count > self.target[1]:
            if self.encoder_count < -40 and self.encoder_count > -80:
                max_stretch_pwm = 600
            else:
                max_stretch_pwm = 600
            ret = int(520 - 0.08*(self.target[0]-self.encoder_count))
...
finger5= finger(v_pin_num=15, g_pin_num=32, enc_pin_num1=22, enc_pin_num2= 23, timer_num=4)
...
#                 finger4.generate_target(message[4])
                finger5.generate_target(message[3])
