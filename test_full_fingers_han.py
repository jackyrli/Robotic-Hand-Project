...
class finger:
    ...
    def control(self, timer):
        ...
        if self.encoder_count>= self.target[0] and self.encoder_count <= self.target[1]:
            ...
            ret = 600 + 7*(self.target[0]-self.encoder_count)
            ret = min(max(700, int(ret + 0.8 * self.error)), 925)
            #print('Bend PWM', ret)
            self.direction = 1
            self.bend(ret)
        elif self.encoder_count > self.target[1]:
            if self.encoder_count < -12 and self.encoder_count > -30:
                max_stretch_pwm = 600
            else:
                max_stretch_pwm = 550
            ret = int(600 - 0.5*(self.target[0]-self.encoder_count))
...
finger5= finger(v_pin_num=4, g_pin_num=21, enc_pin_num1=26, enc_pin_num2=25, timer_num=4)
